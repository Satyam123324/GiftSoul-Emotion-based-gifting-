    ...
    <div style={{ minHeight: '100vh', background: '#FBF7F2' }}>
      ...
      <nav className="gs-nav" style={{
        ...
        <div className="gs-nav-links" style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
          ...
          <Link href="/creators" style={{ fontSize: '.78rem', letterSpacing: '.12em', textTransform: 'uppercase', color: '#B5533C', textDecoration: 'none', fontWeight: 500 }}>Creators</Link>
      ...
      <div className="gs-hero-section" style={{ background: '#2B2019', padding: 'calc(70px + 3rem) 5rem 3rem' }}>
        <p style={{ fontSize: '.72rem', letterSpacing: '.18em', textTransform: 'uppercase', color: '#C99A54', marginBottom: '1rem' }}>The makers behind every gift</p>
        <h1 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: 'clamp(2.2rem,5vw,4.5rem)', fontWeight: 400, color: '#FBF7F2', lineHeight: 1.1, marginBottom: '1rem' }}>
          Real hands.<br /><em style={{ fontStyle: 'italic', color: '#C99A54' }}>Real stories.</em>
        </h1>
      ...
      {/* STATS */}
      <div className="gs-stats" style={{ background: '#F3E8DC', borderBottom: '1px solid #E4D3BE', padding: '1.2rem 5rem', display: 'flex', gap: '3rem', flexWrap: 'wrap' }}>
        {[[`${creators.length}+`, 'verified creators'], ['18', 'states represented'], ['4.9★', 'average rating'], ['2,400+', 'gifts sold']].map(([val, label]) => (
          <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '.6rem' }}>
            <strong style={{ fontSize: '.88rem', color: '#2B2019' }}>{val}</strong>
            <span style={{ fontSize: '.82rem', color: '#7C6B60' }}>{label}</span>
          </div>
        ))}
      </div>
      ...
      <div className="gs-grid-section" style={{ padding: '3rem 5rem 5rem' }}>
        ...
        {!loading && !error && (
          <p style={{ fontSize: '.85rem', color: '#7C6B60', marginBottom: '2rem' }}>Showing {filtered.length} creator{filtered.length !== 1 ? 's' : ''}</p>
        )}

        {loading && (
          <div className="gs-creators-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: '1.8rem' }}>
            {[1, 2, 3, 4].map(i => (
              <div key={i} style={{ background: 'white', border: '1px solid #E4D3BE', borderRadius: '20px', height: '190px', animation: 'gsPulse 1.4s ease-in-out infinite' }} />
            ))}
          </div>
        )}
          ...
          <div className="gs-creators-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: '1.8rem' }}>
            {filtered.map((c, i) => (
              <Link key={c.id || i} href={`/creator/${c.id}`}
                style={{ background: 'white', border: '1px solid #E4D3BE', borderRadius: '20px', padding: '1.6rem', textDecoration: 'none', display: 'block', transition: 'all .25s ease' }}
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
                  {c.photo_url ? (
                    ...
                    <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: '#F3E8DC', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Playfair Display, Georgia, serif', fontSize: '1.4rem', color: '#2B2019', flexShrink: 0 }}>
                      {(c.name || '?')[0].toUpperCase()}
                    </div>
                  )}
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: '1.15rem', color: '#2B2019', marginBottom: '.1rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.name}</div>
                    <div style={{ fontSize: '.7rem', color: '#7C6B60' }}>{c.city}</div>
                  </div>
                  <div style={{ marginLeft: 'auto', textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontSize: '.8rem', color: '#B5533C', fontWeight: 500 }}>★ 4.9</div>
                  </div>
                </div>
                <div style={{ fontSize: '.78rem', color: '#7C6B60', marginBottom: '1rem', paddingBottom: '1rem', borderBottom: '1px solid #E4D3BE' }}>
                  <strong style={{ color: '#2B2019', fontWeight: 500 }}>{c.shop_name || (c.craft_tags || [])[0] || 'Handmade gifts'}</strong>
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.4rem' }}>
                  {(c.emotion_tags || []).slice(0, 3).map(em => (
                    <span key={em} style={{ fontSize: '.68rem', padding: '.22rem .7rem', borderRadius: '2rem', background: 'rgba(181,83,60,.07)', border: '1px solid rgba(181,83,60,.2)', color: '#B5533C' }}>{em}</span>
                  ))}
                </div>
              </Link>
            ))}
