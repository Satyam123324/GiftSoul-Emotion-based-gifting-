    ...
    <div style={{ minHeight: '100vh', background: '#F6F1E9' }}>
      ...
      <div className="gs-product-layout" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', padding: '2rem 5rem 5rem', maxWidth: '1300px', margin: '0 auto' }}>
        ...
        <div className="gs-gallery" style={{ position: 'sticky', top: '90px', height: 'fit-content' }}>
          <div style={{ width: '100%', aspectRatio: '1', background: '#FFF3E0', borderRadius: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '100px', border: '1px solid #D6C2A0' }}>
            🕯️
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '.8rem', marginBottom: '1rem' }}>
            <span style={{ fontSize: '2rem', fontWeight: 500, color: '#241809' }}>₹480</span>
          </div>
          ...
          <div style={{ background: '#ECE2D0', border: '1px solid #D6C2A0', borderRadius: '20px', padding: '1.4rem', marginBottom: '1.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '.8rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
              ...
              <div style={{ fontSize: '.78rem', color: '#5E4E3A' }}>Ready in <strong style={{ color: '#241809' }}>5 working days</strong></div>
            </div>
            <textarea
              value={note}
              ...
              rows={3}
              style={{ width: '100%', border: '1px solid #D6C2A0', borderRadius: '12px', padding: '.7rem 1rem', fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: '1rem', color: '#241809', background: 'white', resize: 'none', outline: 'none', marginBottom: '1rem' }}
            />
            <div style={{ display: 'flex', gap: '.8rem', flexWrap: 'wrap' }}>
              <button onClick={() => setSent(true)} style={{ flex: 1, minWidth: '160px', padding: '.75rem', background: '#A8501F', color: 'white', border: 'none', borderRadius: '2rem', fontSize: '.85rem', cursor: 'pointer', fontWeight: 500 }}>
          ...
          <Link href="/creator-profile" style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.2rem', border: '1px solid #D6C2A0', borderRadius: '20px', background: 'white', textDecoration: 'none', marginBottom: '1.5rem' }}>
            <div style={{ width: '52px', height: '52px', borderRadius: '50%', background: '#FFF3E0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Playfair Display, Georgia, serif', fontSize: '1.3rem', color: '#241809', flexShrink: 0 }}>S</div>
            <div>
              <div style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: '1.1rem', color: '#241809' }}>Sneha Patel</div>
              <div style={{ fontSize: '.72rem', color: '#5E4E3A' }}>Jaipur, Rajasthan · 48 sales</div>
            </div>
            <span style={{ marginLeft: 'auto', color: '#BE9A66', fontSize: '1.1rem' }}>→</span>
          </Link>
          ...
          <div style={{ display: 'flex', borderBottom: '1px solid #D6C2A0', marginBottom: '1.5rem', overflowX: 'auto' }}>
            {[['about', 'About'], ['specs', 'Details'], ['reviews', 'Reviews']].map(([id, label]) => (
              <button key={id} onClick={() => setActiveTab(id)} style={{ padding: '.7rem 1.4rem', fontSize: '.82rem', color: activeTab === id ? '#A8501F' : '#5E4E3A', border: 'none', borderBottom: activeTab === id ? '2px solid #A8501F' : '2px solid transparent', background: 'transparent', cursor: 'pointer', marginBottom: '-1px', whiteSpace: 'nowrap' }}>
                {label}
              </button>
            ))}
          </div>
          ...
          {activeTab === 'reviews' && (
            <p style={{ fontSize: '.9rem', color: '#5E4E3A' }}>★★★★★ 4.9 · 48 reviews</p>
          )}
        </div>
      </div>
